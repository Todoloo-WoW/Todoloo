...
function TodolooTaskManagerFrameMixin:OnLoad()
    ...
    self.managementTabId = self:AddNamedTab("TodolooManagement", self.ManagementPage)
    self.overviewTabId = self:AddNamedTab("TodolooOverview", self.OverviewPage)

    --TODO: Remove - just for testing purposes
    --self:RegisterEvent("TODOLOO_OPEN_TASK")

    --TODO: Implement ability to open task manager on the specific task from the task tracker
    -- EventRegistry:RegisterCallback("Todoloo.TaskManager.SelectTask", function(_, info)
    --     self:SetTaskManagerInfo(info)
    -- end)
end

function TodolooTaskManagerFrameMixin:ApplyDesiredWidth()
    local selectedPage = self:GetElementsForTab(self.managementTabId)[1]
...
function TodolooTaskManagerFrameMixin:SetTabsShown(shown)
    for _, tabID in ipairs(self:GetTabSet()) do
        self.TabSystem:SetTabShown(tabID, shown)
    end
end

function TodolooTaskManagerFrameMixin:UpdateTabs()
    if not self:IsVisible() then
        return
    end

    self:SetTabsShown(true)
    local managementTab = self:GetTabButton(self.managementTabId)
    managementTab.Text:SetText("Management")

    local overviewTab = self:GetTabButton(self.overviewTabId)
    overviewTab.Text:SetText("Overview")

    self.TabSystem:Layout()

    self:SetTab(self:GetTab() or self.managementTabId)
end

function TodolooTaskManagerFrameMixin:GetTaskManagerInfo()
    return Todoloo.TaskManager.GetTaskManagerInfo()
end

function TodolooTaskManagerFrameMixin:SetTaskManagerInfo(taskManagerInfo)
    self.taskManagerInfo = Todoloo.TaskManager.GetTaskManagerInfo()
    self.taskManagerInfo = taskManagerInfo.openTaskId

    self:Refresh()
end

function TodolooTaskManagerFrameMixin:Refresh()
    --TODO: Correctly set texture
    self:SetPortraitToAsset("Interface\\AddOns\\Todoloo\\Images\\Logo")

    for _, page in ipairs(self.Pages) do
        page:Refresh(self.taskManagerInfo)
    end
...
function TodolooTaskManagerFrameMixin:OnEvent(event, ...)
    if event == "TODOLOO_OPEN_TASK" then
        local taskId = ...

        --TODO: To be continued... (see line 142 in Blizzard_ProffesionsFrame.lua)
    end
end
...
-- #####################################################################################################
-- ***** BELOW IS TASK MANAGE BEFORE IMPROVEMENT - SHOWS IN ADDON SETTINGS
-- #####################################################################################################
...
--     local self = TodolooTaskManagerFrame
